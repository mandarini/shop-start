...
/**
 * 
 * In this function, when a user requests a product,
 * we want to go and collect all comments for that product and
 * add them in the original product object.
 * 
 * Then return that object to our user.
 * 
 * We do this because, as we said, Firestore is a NoSQL, non 
 * relational database. We cannot make compound queries as we do in SQL.
 * 
 * Also, when we are fetching a document, we do not fetch its subcollections.
 * 
 * So, we need to craft this query on our own.
 */
...
/**
 * 
 * For each comment, we go and fetch each
 * user's display name, too.
 */
